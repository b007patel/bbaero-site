...
// BP - since using 5.5.9 commented make_seed() line below
//$seed_func(make_seed());
...
$bg_fade_pct += $GLOBALS['rand_func'](-2,2);
...
if(empty($_SESSION['freecap_attempts']))
{
	...
	if($_SESSION['freecap_attempts']>$max_attempts)
	{
		...
		$bg1 = ImageColorAllocate($GLOBALS['im1'],255,255,255);
		ImageColorTransparent($GLOBALS['im1'],$bg1);

		$err_color = ImageColorAllocate($GLOBALS['im1'], 0, 255, 0);
		// depending on how rude you want to be :-)
		//ImageString($GLOBALS['im1'],5,0,20,"bugger off you spamming bastards!",$err_color);
		ImageString($GLOBALS['im1'],5,15,20,"service no longer available",$err_color);
...
function rand_color() {
	if($GLOBALS['bg_type']==3)
	{
		// needs darker colour..
		return $GLOBALS['rand_func'](10,100);
	} else {
		//return $GLOBALS['rand_func'](60,170);
		return $GLOBALS['rand_func'](60,140);
	}
}
...
function myImageBlur($im)
{
	...
	$bg1 = ImageColorAllocate($temp_im,150,150,150);

	// preserves transparency if in orig image
	ImageColorTransparent($temp_im,$bg1);

	// fill bg
	ImageFill($temp_im,0,0,$bg1);
...
function sendImage($pic)
{
	...
	ImageDestroy($GLOBALS['im1']);
	ImageDestroy($GLOBALS['im2']);
	//ImageDestroy($pic);
	if(!empty($GLOBALS['im3']))
	{
		ImageDestroy($GLOBALS['im3']);
	}
	exit();
}
...
if($use_dict==1)
{
	...
	$wordlen = $GLOBALS['rand_func'](5,$max_word_length);

	for($i=0 ; $i<$wordlen ; $i++)
	{
		// don't allow to start with 'vowel'
		if($GLOBALS['rand_func'](0,4)>=2 && $i!=0)
		{
			$word .= $vowels{$GLOBALS['rand_func'](0,strlen($vowels)-1)};
		} else {
			$word .= $consonants{$GLOBALS['rand_func'](0,strlen($consonants)-1)};
		}
	}
}
...
$_SESSION['bp_dbg_word'] = $word;
//$_SESSION['freecap_word_hash'] = $hash_func(strtolower($word));
...
$tag_col = ImageColorAllocate($GLOBALS['im1'],10,10,10);
$site_tag_col2 = ImageColorAllocate($GLOBALS['im2'],0,0,0);

// set debug colours (text colours are set later)
$debug = ImageColorAllocate($GLOBALS['im1'], 255, 0, 0);
$debug2 = ImageColorAllocate($GLOBALS['im2'], 255, 0, 0);
...
$bg1 = ImageColorAllocate($GLOBALS['im1'], 254, 254, 254);
$bg2 = ImageColorAllocate($GLOBALS['im2'], 254, 254, 254);

// set transparencies
ImageColorTransparent($GLOBALS['im1'],$bg1);
// im2 transparent to allow characters to overlap slightly while morphing
ImageColorTransparent($GLOBALS['im2'],$bg2);

// fill backgrounds
ImageFill($GLOBALS['im1'],0,0,$bg1);
ImageFill($GLOBALS['im2'],0,0,$bg2);
...
if($GLOBALS['bg_type']!=0)
{
	...
	$GLOBALS['im3'] = ImageCreateTrueColor($width,$height);
	$temp_bg = ImageCreateTrueColor($width*1.5,$height*1.5);
	$bg3 = ImageColorAllocate($GLOBALS['im3'],255,255,255);
	ImageFill($GLOBALS['im3'],0,0,$bg3);
	...
	// or if not, just copy a $widthx$height portion of $temp_bg to $GLOBALS['im3']
	...
	if($GLOBALS['bg_type']==1)
	{
		...
		// draw grid on x
		for($i=$GLOBALS['rand_func'](6,20) ; $i<$width*2 ; $i+=$GLOBALS['rand_func'](10,25))
		{
			ImageSetThickness($temp_bg,$GLOBALS['rand_func'](2,6));
			$text_r = $GLOBALS['rand_func'](100,150);
			$text_g = $GLOBALS['rand_func'](100,150);
			$text_b = $GLOBALS['rand_func'](100,150);
			$text_colour3 = ImageColorAllocate($temp_bg, $text_r, $text_g, $text_b);

			ImageLine($temp_bg,$i,0,$i,$height*2,$text_colour3);
		}
		// draw grid on y
		for($i=$GLOBALS['rand_func'](6,20) ; $i<$height*2 ; $i+=$GLOBALS['rand_func'](10,25))
		{
			ImageSetThickness($temp_bg,$GLOBALS['rand_func'](2,6));
			$text_r = $GLOBALS['rand_func'](100,150);
			$text_g = $GLOBALS['rand_func'](100,150);
		...
		for($i=0 ; $i<strlen($word)+1 ; $i++)
		{
			...
			for($j=1 ; $j<$GLOBALS['rand_func'](5,10) ; $j++)
			{
				$points[] = $GLOBALS['rand_func'](1*(20*($i+1)),1*(50*($i+1)));
				$points[] = $GLOBALS['rand_func'](30,$height+30);
			}

			ImagePolygon($temp_bg,$points,intval(sizeof($points)/2),$text_colour3);
		}

	} else if($GLOBALS['bg_type']==3) {
		...
		$blocksize = $GLOBALS['rand_func'](20,60);
		for($i=0 ; $i<$width*2 ; $i+=$blocksize)
		{
			// could randomise blocksize here... hardly matters
			for($j=0 ; $j<$height*2 ; $j+=$blocksize)
			{
				$image_index = $GLOBALS['rand_func'](0,sizeof($temp_im)-1);
				$cut_x = $GLOBALS['rand_func'](0,$temp_width[$image_index]-$blocksize);
				$cut_y = $GLOBALS['rand_func'](0,$temp_height[$image_index]-$blocksize);
	...
	if($morph_bg)
	{
		...
		$morph_chunk = $GLOBALS['rand_func'](1,5);
		$morph_y = 0;
		for($x=0 ; $x<$width ; $x+=$morph_chunk)
		{
			$morph_chunk = $GLOBALS['rand_func'](1,5);
			$morph_y += $GLOBALS['rand_func'](-1,1);
			ImageCopy($GLOBALS['im3'], $temp_bg, $x, 0, $x+30, 30+$morph_y, $morph_chunk, $height*2);
		}

		ImageCopy($temp_bg, $GLOBALS['im3'], 0, 0, 0, 0, $width, $height);

		$morph_x = 0;
		for($y=0 ; $y<=$height; $y+=$morph_chunk)
		{
			$morph_chunk = $GLOBALS['rand_func'](1,5);
			$morph_x += $GLOBALS['rand_func'](-1,1);
			ImageCopy($GLOBALS['im3'], $temp_bg, $morph_x, $y, 0, $y, $width, $morph_chunk);

		}
	} else {
		// just copy temp_bg onto im3
		ImageCopy($GLOBALS['im3'],$temp_bg,0,0,30,30,$width,$height);
	}
...
$word_start_x = $GLOBALS['rand_func'](5,32);
...
for($i=0 ; $i<strlen($word) ; $i++)
{
	...
	ImageString($GLOBALS['im2'], $font, $word_start_x+($font_widths[$j]*$i), $word_start_y, $word{$i}, $text_colour2);
}
// use last pixelwidth
$font_pixelwidth = $font_widths[$j];

// for debug:
//sendImage($GLOBALS['im2']);
...
for($i=$word_start_x ; $i<$word_pix_size ; $i+=$font_pixelwidth)
{
	...
	$prev_y = $y_pos;
	do{
		$y_pos = $GLOBALS['rand_func'](-5,5);
	} while($y_pos<$prev_y+2 && $y_pos>$prev_y-2);
	ImageCopy($GLOBALS['im1'], $GLOBALS['im2'], $i, $y_pos, $i, 0, $font_pixelwidth, $height);

	// for debug:
	//ImageRectangle($GLOBALS['im1'],$i,$y_pos+10,$i+$font_pixelwidth,$y_pos+70,$debug);
}
...
for($j=0 ; $j<strlen($word) ; $j++)
{
	...
	for($i=0 ; $i<=$height; $i+=$y_chunk)
	{
		...
		$morph_x += $GLOBALS['rand_func'](-$morph_factor,$morph_factor);
		...
		ImageCopyMerge($GLOBALS['im2'], $GLOBALS['im1'], $orig_x+$morph_x, $i+$y_pos, $orig_x, $i, $font_pixelwidth, $y_chunk, 100);

		// for debug:
		//ImageLine($GLOBALS['im2'], $orig_x+$morph_x, $i, $orig_x+$morph_x+1, $i+$y_chunk, $debug2);
		//ImageLine($GLOBALS['im2'], $orig_x+$morph_x+$font_pixelwidth, $i, $orig_x+$morph_x+$font_pixelwidth+1, $i+$y_chunk, $debug2);
	}
}
...
ImageFilledRectangle($GLOBALS['im1'],0,0,$width,$height,$bg1);
...
if(is_array($site_tags))
{
	for($i=0 ; $i<sizeof($site_tags) ; $i++)
	{
		...
		{
			// write at top
			ImageString($GLOBALS['im2'], 2, intval($width/2)-intval($tag_width/2), (10*$i), $site_tags[$i], $site_tag_col2);
		}
		if($tag_pos==1 || $tag_pos==2)
		{
			// write at bottom
			ImageString($GLOBALS['im2'], 2, intval($width/2)-intval($tag_width/2), ($height-34+($i*10)), $site_tags[$i], $site_tag_col2);
		}
	}
}
ImageCopyMerge($GLOBALS['im2'],$GLOBALS['im1'],0,0,0,0,$width,$height,80);
ImageCopy($GLOBALS['im1'],$GLOBALS['im2'],0,0,0,0,$width,$height);
// for debug:
//sendImage($GLOBALS['im1']);
...
if($GLOBALS['bg_type']!=0)
{
	...
	if($GLOBALS['bg_type']!=3)
	{
		...
		ImageCopyMerge($GLOBALS['im3'],$temp_im,0,0,0,0,$width,$height,$bg_fade_pct);
		// for debug:
		//sendImage($GLOBALS['im3']);
		ImageDestroy($temp_im);
		$c_fade_pct = 50;
	...
	if($merge_type==1)
	{
		ImageCopyMerge($GLOBALS['im3'],$GLOBALS['im1'],0,0,0,0,$width,$height,100);
		ImageCopy($GLOBALS['im1'],$GLOBALS['im3'],0,0,0,0,$width,$height);
	} else {
		// bg over captcha:
		ImageCopyMerge($GLOBALS['im1'],$GLOBALS['im3'],0,0,0,0,$width,$height,$c_fade_pct);
	}
}
// for debug:
//sendImage($GLOBALS['im1']);
